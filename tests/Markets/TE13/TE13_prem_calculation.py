...
spot_df = get_spot_price_fr(start_date = f"{cal_year}-01-01", end_date = f"{cal_year}-12-31")
forward_df = get_forward_price_fr_annual(cal_year=cal_year)
pfc_df = get_pfc_fr(price_date= cal_year-1 , delivery_year=cal_year)
...
calculate_prem_risk_vol(forecast_df=forecast_df,spot_df=spot_df,forward_df=forward_df,quantile=70, plot_chart=True)
